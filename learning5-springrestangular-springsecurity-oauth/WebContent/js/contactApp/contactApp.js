...
contactApp.controller("UserController", function($scope, $http) {
	...
	$scope.getAll = function() {
		...
		var encodedString = encode('my-trusted-client:secret');	
		alert("encodedString: " + encodedString);
		var accessToken = '';
				
		$http({
			url : "oauth/token?grant_type=password&username=bill&password=abc123",
			method : 'POST',
			headers : {
				...
				var url2 = 'users/rest/getallusers/?access_token='+accessToken;
				alert("url2: "  + url2);
				$http.get(url2)
					.success(function(data) {				
						$scope.users = data;
		...
		var url2 = 'users/rest/getallusers/?access_token='+accessToken;
		alert("url2: "  + url2);
		$http.get('users/rest/getallusers/?access_token='+accessToken)
			.success(function(data) {				
				$scope.users = data;
			});
			*/
	...
	$scope.getCountries = function() {		
		...
		$http({
			...
			headers : {
				...
				var url2 = 'users/rest/getallcountries/?access_token='+accessToken;
				alert("url2: "  + url2);
				$http.get(url2)
					.success(function(data) {				
						$scope.countries = data;
